	...
	public static List<PrivateStock> jsonGetPrivateStock(JSONArray jsonArray){
		...
		for(int i=0;  i< jsonArray.size();i++){
			...
			try{
				...
				privateStock.setPrice_date(price_date);
			}catch (Exception e) {
				// TODO: handle exception
				privateStock.setNav(nullValue);
				privateStock.setPrice_date(nullString);
			}
			...
			try{
				//累积
				Double cumulative_nav = jsonArray.getJSONObject(i).getDouble("ret_incep") ;
			...
			//年化率
			privateStock.setYearNav(nullValue);
			//夏普比率
			privateStock.setSharperPercent(nullValue);
			
			try{
				//一年评级
				int profit_col5 = jsonArray.getJSONObject(i).getInt("profit_col5");
				privateStock.setProfit_col5(profit_col5);
			}catch (Exception e) {
				// TODO: handle exception
				int a = 0;
				privateStock.setProfit_col5(a);
			}
			...
			privateStocks.add(privateStock);
