...
import com.iparhan.spider.dao.MysqlFOFFund;
import com.iparhan.spider.entity.FOFFund;
import com.iparhan.spider.utils.IOSave;
import com.iparhan.spider.utils.JsonGenFOFFund;
...
public class FOFFundController {
			...
			public String spiderCompany(String username, String message) throws ClientProtocolException, IOException, SQLException{
				...
				//爬取的数据存储本地
				String fileName = "fofFund";
				IOSave.saveDataToFile(fileName, companyData);
				System.out.println("====================是否执行到此步骤噢==================");
				
				//解析和存储到数据库
				//解析JSON数据
				List<FOFFund> threeMonths = JsonGenFOFFund.jsonGetFOFFund(companyData);
				MysqlFOFFund.executeUpdateFOFFund(threeMonths);
