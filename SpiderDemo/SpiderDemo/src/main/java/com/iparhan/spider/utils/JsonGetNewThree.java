...
public class JsonGetNewThree {
	
	/*
	 * JSON数据解析使用的
	 */
	public static List<NewThreeFund> jsonGetNewThreeFund(JSONArray jsonArray){
		//使用集合括起来
		List<NewThreeFund>  newThreeFunds = new ArrayList<NewThreeFund>();
		
		//遍历
		for(int i=0;  i< jsonArray.size();i++){
			//私募排汗实体类 使用于爬过来的数据封装成对象
			NewThreeFund overSeasFund = new NewThreeFund();
			...
			try{
				...
				System.out.println("新三板最新净值日期"+price_date);
			}catch (Exception e) {
			...
			try{
				//累积净值
				Double cumulative_nav = jsonArray.getJSONObject(i).getDouble("profit_col3");
			...
			newThreeFunds.add(overSeasFund);
